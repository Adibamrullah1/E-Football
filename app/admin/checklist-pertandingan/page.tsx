...
export const metadata: Metadata = {
  title: 'Ceklist Pertandingan | Admin',
}

export default async function AdminChecklistPertandinganPage({ searchParams }: { searchParams: { season?: string } }) {
  ...
  if (!targetSeason) {
    return (
      <div className="space-y-4 md:space-y-6">
        <h1 className="font-heading text-2xl md:text-3xl font-bold text-foreground">Ceklist Pertandingan</h1>
  ...
  const matches = await prisma.match.findMany({
    where: { seasonId: targetSeason.id, status: 'FINISHED', isVerified: true },
    select: {
      id: true,
      status: true,
      homeScore: true,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground mr-1">
            {matches.length} pertandingan telah diverifikasi
          </p>
        </div>
      </div>
